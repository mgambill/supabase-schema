<template>
  <teleport to="body">
    <div v-if="open"
      class="w-screen h-screen fixed top-0 left-0 flex items-center justify-center bg-zinc-400 dark:bg-zinc-900 !bg-opacity-50">
      <div ref="target"
        class="bg-zinc-100 dark:bg-zinc-800 w-full max-w-screen-md h-screen-sm rounded-md border-2 dark:border-dark-600 flex flex-col">
        <div
        ...
        <div class="p-4 text-dark-100 dark:text-white h-full overflow-hidden overflow-y-auto">
          ...
          <pre
            class="bg-zinc-200 dark:bg-zinc-900 text-zinc-500 dark:text-white-800 text-sm rounded-md p-4 h-auto">{{ exportedCode }}</pre>
        </div>
      </div>
    </div>
  </teleport>
